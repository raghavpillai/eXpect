...
async def sample_user_followers(user_id: str, max_sample: int = Config.MAX_FOLLOWER_SAMPLE) -> List[dict]:
    ...
    try:
        ...
        follower_ids = set(user['id'] for user in followers['data'] if user and 'id' in user)
        following_ids = set(user['id'] for user in following['data'] if user and 'id' in user)
...
async def sample_users_with_tweets_from_username(username: str, save_sample: bool = True) -> UserSampleResponse:
    ...
    try:
        ...
        user_with_tweets_list = [uwt for uwt in user_with_tweets_list if uwt is not None]
